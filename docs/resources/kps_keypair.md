...
```hcl
resource "huaweicloud_kps_key" "test" {
  key_alias = "kms_test"
}
...
resource "huaweicloud_kps_keypair" "test-keypair" {
  ...
  scope                   = "domain"
  encryption_type         = "kms"
  encryption_kms_key_name = huaweicloud_kps_key.test.key_alias
}
...
* `type` - (Optional, String, ForceNew) Specifies the type of SSH key pair. Valid values include: `ssh` and `x509`.
 Changing this parameter will create a new resource.

* `scope` - (Optional, String, ForceNew) Specifies the scope of key pair. The options are as follows:
  - **domain**: Tenant-level, available to all users under the same account.
...
* `encryption_kms_key_name` - (Optional, String, ForceNew) Specifies the KMS key name to encrypt private keys.
...
and `encryption_kms_key_name`. It is generally recommended running `terraform plan` after importing a key pair.
...
resource "huaweicloud_kps_keypair" "test" {
  ...
  lifecycle {
    ignore_changes = [
      encryption_type, encryption_kms_key_name,
    ]
  }
}
```
